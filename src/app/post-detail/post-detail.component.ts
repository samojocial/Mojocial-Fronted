...
export class PostDetailComponent implements OnInit {
  ...
  onAddComment(comment: { value: { content: string; postId: number; username: string; }; }, post: Post) {
    this.comment.content = '';
  ...
  displayLike(user: User) {
    ...
    const result  =    user.likedPost.find(post => post.id === this.post.id);
    if (result) {
      this.like = 'Unlike';
      this.color = '#18BC9C';
      console.log('testing');
...
/*
  likePost(post: { likes: number; id: any; }, user: { likedPosts: any[] | null; }) {
  ...
  doLike(post: { id: number; }, user: { username: string }) {
    this.subscriptions.push(
  ...
  */
 doUnlike(post: { id: number; }, user: { username: string; }) {
    this.subscriptions.push(
